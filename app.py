...
def process_all(sb,cb,lb):
    ...
    # ── LEADS ─────────────────────────────────────────────────────────────────
    leads=pd.read_excel(io.BytesIO(lb),sheet_name='Sheet 1')
...
{sc("Lead Owners",f"{leads['leadownername'].nunique()}","Following leads",P,"n")}
        ...
        with ov3:
            ...
            mo=conv.groupby('month').agg(n=('_id','count'),rev=('total_amount','sum')).reset_index().tail(6)
...
with t_leads:
    if not st.session_state.loaded:
        ...
        with st.expander("🔽  Leads Filters",expanded=True):
            ...
            min_d=leads_all['leaddate'].min().date(); max_d=leads_all['leaddate'].max().date()
        ...
        if sel_att!="All": ld=ld[ld['Attempted/Unattempted']==sel_att]
...
{sc("Campaigns",f"{ld['campaign_name'].nunique()}","Active",B,"n")}
        ...
        with o4:
            ...
            fig9a.update_layout(**BASE,height=150,showlegend=False,margin=dict(l=0,r=0,t=4,b=4))
            ...
            fig9b.update_layout(**BASE,height=115,bargap=0.45,margin=dict(l=0,r=40,t=4,b=0),xaxis=dict(showgrid=False,showticklabels=False,zeroline=False,range=[0,atc['Rate'].max()*1.4] if len(atc) else [0,10]),yaxis=dict(showgrid=False,tickfont=dict(size=10,color=TXT2)))
...
{sc("Short Closed",f"{int(sc_cv)}","",A,"a")}
        ...
        with cv6:
            ...
            fig6=go.Figure(go.Bar(x=pm_cv.values,y=pm_cv.index,orientation='h',marker_color=[G,B,P,A,R,T,GR][:len(pm_cv)],marker_line_width=0,text=pm_cv.values,textposition='outside',textfont=dict(size=9,color=TXT2),hovertemplate='<b>%{y}</b><br>%{x}<extra></extra>'))
